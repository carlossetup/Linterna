...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnLinterna = (Button) findViewById(R.id.btnLinterna);//Busca el id del boton al cual le queremos dar una accion
        if (!tieneflash){
            btnLinterna.setEnabled(false);//Si no tiene flash desabilita el boton
        ...
        if (!linternaON){
            if (camera == null){//si la camara no inicio
                try{
                    ...
                    btnLinterna.setEnabled(false);//desactiva el botn
                }
            }
